    ...
    def gen_func(api_group_name, f_name, f_url):
        ...
        def infer_request_method():
            ...
            for k, v in api_config.keywords.items():
                if action in v:
                    return k
            raise ValueError(
                "Unknown Keyword '{}' in '{}'".format(action, f_name))
        ...
        def func(self, *args, **kwargs):
            ...
            for kp in keyword_params:
                ...
                kp_value = kp[1] if kwargs.get(
                    kp[0]) is None else kwargs.get(kp[0])
                request_url = re.sub(
                    "\{" + kp[0] + ":[^{}]+?\}", kp_value, request_url)

            # 处理url中位置参数,既{key}格式.将{key}替换为{},再用*args填充.
            request_url = re.sub("\{([^:]*?)\}", "{}", request_url)
            request_url = "/".join([self.base_url,
                                    api_group_name, request_url.format(*args)])
